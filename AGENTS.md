...
```
Invidious (invidious.awa.3d.ae.net.nz) ←── server.py (/api/invidious/*) ←── download_assets.py
...
- `.claude/launch.json` references `/Users/pleathen/Projects/fifa-wc-2026-wallpaper` — stale macOS path, update if using Claude Code launch
...
- Invidious proxy in server.py requires invidious.awa.3d.ae.net.nz to be reachable (LAN)
...
| Remote | URL | Purpose |
|--------|-----|---------|
| `origin` | `https://git.awa.3d.ae.net.nz/aenertia/wc2026.git` | Forgejo dev repo |
